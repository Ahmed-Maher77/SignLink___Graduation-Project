...
async def process_frames_worker(client_session: ClientSession, current_config: AppConfig):
    ...
    async def process_grammar_results():
        while True:
            try:
                task, sentence = await grammar_tasks.get()
                try:
                    result = await task
                ...
                try:
                    ...
                    if result:  
                        ...
                        if is_special_case:
                            ...
                            if (
                                needs_api_call and
                                current_config.OPENROUTER_API_KEY and
                                current_original_str not in client_session.grammar_in_flight
                            ):
                                client_session.grammar_in_flight.add(current_original_str)
                                grammar_task = asyncio.create_task(
                                    asyncio.to_thread(
                                        get_api_corrected_sentence, current_original_str, current_config
                                    )
                                )
                                await grammar_tasks.put((grammar_task, current_original_str))
